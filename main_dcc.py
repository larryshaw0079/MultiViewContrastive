...
from mvc.model import DCC, DCCClassifier
    ...
    for epoch in range(args.finetune_epochs):
        ...
        with tqdm(data_loader, desc=f'EPOCH [{epoch + 1}/{args.finetune_epochs}]') as progress_bar:
            for x, y in progress_bar:
                ...
                out = classifier(x)
                loss = criterion(out.view(args.batch_size * args.num_epoch, -1), y.view(-1))
...
def evaluate(classifier, dataset, device, args):
    ...
    with torch.no_grad():
        for x, y in data_loader:
            ...
            out = classifier(x)
            scores.append(out.view(args.batch_size * args.num_epoch, -1).cpu().numpy())
...
def main_worker(run_id, device, train_patients, test_patients, args):
    model = DCC(input_channels=args.channels, hidden_channels=16, feature_dim=args.feature_dim,
                use_temperature=args.use_temperature, temperature=args.temperature,
                device=device)
    model.cuda(device)
    ...
    torch.save(model.state_dict(), os.path.join(args.save_path, f'dcc_run_{run_id}_pretrained.pth.tar'))
    ...
    classifier = DCCClassifier(input_channels=args.channels, hidden_channels=16,
                               feature_dim=args.feature_dim, num_class=args.classes,
    ...
    torch.save(model.state_dict(), os.path.join(args.save_path, f'dcc_run_{run_id}_finetuned.pth.tar'))
