...
from mvc.data import SleepDataset, SleepDatasetSampling
from mvc.model import RelativePosition, TemporalShuffling, SimpleClassifier
...
def parse_args(verbose=True):
    ...
    # Model
    parser.add_argument('--model', type=str, default='rp', choices=['rp', 'ts'])
    parser.add_argument('--num-sampling', type=int, required=True)
    parser.add_argument('--dis', type=int, required=True)
    ...
    for epoch in range(start_epoch, args.pretrain_epochs):
        ...
        if (epoch + 1) % args.save_interval == 0:
            torch.save({'state_dict': model.state_dict(), 'epoch': epoch},
                       os.path.join(args.save_path, f'{args.model}_run_{run_id}_pretrain_{epoch}.pth.tar'))
...
def main_worker(run_id, device, train_patients, test_patients, args):
    if args.model == 'rp':
        model = RelativePosition(input_channels=args.channels, hidden_channels=16, feature_dim=args.feature_dim,
                                 device=device)
    else:
        model = TemporalShuffling(input_channels=args.channels, hidden_channels=16, feature_dim=args.feature_dim,
                                  device=device)
    model.cuda(device)

    train_dataset = SleepDatasetSampling(args.data_path, args.data_name, args.num_sampling,
                                         'pair' if args.model == 'rp' else 'triplet', args.dis,
    ...
    classifier = SimpleClassifier(input_channels=args.channels, hidden_channels=16,
